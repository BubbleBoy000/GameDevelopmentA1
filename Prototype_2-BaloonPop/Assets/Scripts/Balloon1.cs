...
{
    public int clicksToPop = 1; // Number of clicks required to pop the balloon
    ...
    void OnMouseDown()
    {
        ...
        if (clicksToPop == 0)
        {
            // Find the SpawnAlotTrap object and call its method
            GameObject trapObj = GameObject.Find("SpawnAlotTrap");
            if (trapObj != null)
            {
                SpawnAlotTrap trap = trapObj.GetComponent<SpawnAlotTrap>();
                if (trap != null)
                {
                    trap.SpawnRandomBalloons();
                }
            }
